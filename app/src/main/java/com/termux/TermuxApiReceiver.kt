...
class TermuxApiReceiver : BroadcastReceiver() {
    ...
    private fun showNotification(context: Context, args: String): String {
        ...
        val priorityStr = parts.getOrNull(3) ?: "default"
        val ongoing = parts.getOrNull(4) == "true"
        val sound = parts.getOrNull(5) == "true"

        val notificationManager = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

        // Map priority string to NotificationCompat priority
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val importance = when (priorityStr.lowercase()) {
                "high", "max" -> NotificationManager.IMPORTANCE_HIGH
                "low" -> NotificationManager.IMPORTANCE_LOW
                "min" -> NotificationManager.IMPORTANCE_MIN
                else -> NotificationManager.IMPORTANCE_DEFAULT
            }
            val channel = NotificationChannel(CHANNEL_ID, "Termux API", importance)
