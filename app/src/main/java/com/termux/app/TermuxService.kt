 ...
 *
 * This is the CRITICAL missing piece for overnight/background use.
 * Provides:
 * - Wake lock support (CPU stays awake)
 * - WiFi lock support (network stays connected)
 * - Session persistence (sessions survive activity lifecycle)
...
class TermuxService : Service() {
    ...
    // Session management - thread-safe list for concurrent access
    private val sessions = Collections.synchronizedList(mutableListOf<TerminalSession>())
    ...
    override fun onDestroy() {
        ...
        stopAmSocketServer()
        releaseWakeLock()
        // Clean up sessions - synchronized iteration for thread safety
